...
function setlinkageareas(v){
	var str="0";
	for(i=0; i<v; i++){str+=("_"+(document.getElementById(linkageareaids[i]).selectedIndex-1));}
	var ss = document.getElementById(linkageareaids[v]);
	with(ss){
		length = 0;
		options[0] = new Option(linkageareadefvalues[v],linkageareadefvalues[v]);
		if(v && document.getElementById(linkageareaids[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
		...
		if(++v<linkageareaids.length){setlinkageareas(v);}
	}
}
